...
def boot():
    ...
    if not os.getenv("TURSO_DATABASE_URL", "").strip():
        logger.warning("=" * 60)
        logger.warning("WARNING: TURSO_DATABASE_URL is not set.")
        logger.warning("Memory logging, pattern tracking, and resonance")
        logger.warning("detection are DISABLED. Sol Calarbone 8 will not")
...
# In-memory cache for files generated by the Solar8 generate_file tool.
# Keys are UUIDs; values are dicts with bytes/filename/mimetype.
# Capped at 100 entries (FIFO eviction) to prevent unbounded memory growth.
_generated_file_cache: dict = {}
_FILE_CACHE_MAX = 100
...
def _build_file_bytes(content: str, filename: str, fmt: str) -> tuple[bytes, str]:
    """
    Build file bytes and MIME type for the given format.
    Returns (file_bytes, mime_type).
    """
    if fmt == "html":
        title = _html.escape(filename.replace("_", " ").replace("-", " "))
...
def _safe_download_name(filename: str, fmt: str) -> str:
    """Return a sanitized download filename, stripping characters unsafe for headers."""
    import re
    safe = re.sub(r'[^\w.\-]', '_', filename)
    return safe if safe.endswith(f".{fmt}") else f"{safe}.{fmt}"
...
def generate_file():
    ...
    fmt = data.get("format", "md").strip().lower()
    if not content or not filename:
        return jsonify({"status": "error", "message": "content and filename required."}), 400
    if fmt not in ("md", "txt", "html"):
        return jsonify({"status": "error", "message": "format must be md, txt, or html."}), 400
    try:
        ...
        logger.error("generate_file error: %s", e)
        return jsonify({"status": "error", "message": "File generation failed. Check logs."}), 500


@app.route("/api/generate-file/<token>")
def serve_generated_file(token):
    """Serve a file previously generated by the Solar8 generate_file tool."""
    entry = _generated_file_cache.get(token)
    if not entry:
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "File serve failed. Check logs."}), 500
